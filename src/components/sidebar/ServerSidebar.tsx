...
import { FolderHeart, Star as StarIcon, CheckSquare } from 'lucide-react';
...
export const ServerSidebar: React.FC = () => {
    ...
    // M3: smart folders — saved tag+sort view presets
    const { folders: smartFolders, addFolder, removeFolder } = useSmartFolders();
    // M3: bulk selection mode
    ...
    const handleWakeOnLan = useCallback(
        async (connection: ServerConnection) => {
            setContextMenu(null);
            // Use the saved MAC if present; otherwise ask. Subnet broadcast is
            // derived backend-side (defaults to 255.255.255.255).
            const mac = connection.mac_address?.trim()
                || (await prompt({
                    title: `Wake "${connection.name}"`,
                    label: 'Enter the target MAC address.',
                    placeholder: 'AA:BB:CC:DD:EE:FF',
                    confirmLabel: 'Send',
                    validate: v =>
                        /^([0-9a-fA-F]{2}[:-]?){5}[0-9a-fA-F]{2}$/.test(v.replace(/\s/g, ''))
                            ? null
                            : 'Invalid MAC address',
                }));
            if (!mac) return;
            try {
                await api.wakeOnLan(mac.trim());
                addToast({ type: 'success', title: 'Magic packet sent', description: `Wake-on-LAN sent to ${connection.name}.` });
            } catch (err) {
                addToast({ type: 'error', title: 'Wake-on-LAN failed', description: String(err) });
            }
        },
        [addToast]
    );
        ...
        (item: FlatItem, style: React.CSSProperties) => {
            ...
            return (
                ...
                >
                    ...
                    <div className={selectMode && selectedIds.has(conn.id) ? 'rounded-lg ring-2 ring-accent ring-inset' : ''}>
                        <ConnectionItem
                            connection={conn}
                            ...
                            isActive={isActive}
                            onOpen={() => {
                                if (selectMode) { toggleSelect(conn.id); return; }
                                if (!justDragged.current) openTab(conn);
                            }}
                            onEdit={() => {
    ...
    return (
        <div className="w-64 flex flex-col h-full bg-surface border-r border-border select-none">
            ...
            <div className="px-3 pb-2 shrink-0 space-y-2">
                <div className="flex gap-1 items-center">
                    ...
                    <button
                        ...
                        className={`shrink-0 p-1 rounded transition-colors ${selectMode ? 'bg-accent text-white' : 'bg-base text-text-muted hover:text-text-primary'}`}
                    >
                            ...
                            <button
                                onClick={async () => {
                                    const name = await prompt({
                                        ...
                                        label: `A view filtered by tag "${tagFilter}".`,
                                        placeholder: 'Folder name',
                                        defaultValue: tagFilter,
                                        confirmLabel: 'Save',
                                    });
                                    if (name?.trim()) addFolder(name.trim(), tagFilter, sortMode);
                                }}
                                title="Save current filter as a smart folder"
                                className="flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-semibold bg-accent/10 text-accent hover:bg-accent/20"
                            >
                                <StarIcon className="w-2.5 h-2.5" /> Save
                            </button>
                        )}
                    </div>
                )}
                {smartFolders.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                        {smartFolders.map(sf => (
                            <button
                                key={sf.id}
                                onClick={() => { setTagFilter(sf.tag); setSortMode(sf.sortMode); }}
                                onContextMenu={e => { e.preventDefault(); removeFolder(sf.id); }}
                                title={`${sf.name} (tag: ${sf.tag}) — right-click to remove`}
                                className={`flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-semibold transition-colors ${tagFilter === sf.tag ? 'bg-violet-500 text-white' : 'bg-violet-500/10 text-violet-300 hover:bg-violet-500/20'}`}
                            >
                                <FolderHeart className="w-2.5 h-2.5" />{sf.name}
                            </button>
                        ))}
                    </div>
                )}
            ...
            {selectMode && selectedIds.size > 0 && (
                <div className="shrink-0 mx-2 mb-2 p-2 rounded-xl bg-surface border border-accent/30 flex items-center gap-2 shadow-lg">
                    <span className="text-xs font-bold text-accent px-1">{selectedIds.size} selected</span>
                    <select
                        defaultValue=""
                        onChange={async e => {
                            const gid = e.target.value === '__root__' ? null : e.target.value;
                            if (e.target.value === '') return;
                            const moved = new Set(selectedIds);
                            for (const id of moved) {
                                try { await api.updateConnectionGroup(id, gid); } catch { /* skip */ }
                            }
                            // Optimistic: patch group_id in the store instead of a full re-fetch.
                            const { connections: cur, setConnections } = useConnectionStore.getState();
                            setConnections(cur.map(c => moved.has(c.id) ? { ...c, group_id: gid } : c));
                            exitSelectMode();
                        }}
                        className="flex-1 min-w-0 h-7 bg-base border border-border rounded text-xs px-1 outline-none"
                    >
                    ...
                    <button
                        onClick={async () => {
                            const ok = await confirm(`Delete ${selectedIds.size} connection(s)?`, { title: 'Confirm Delete', kind: 'warning' });
                            if (!ok) return;
                            for (const id of selectedIds) {
                                try { await deleteConnection(id); } catch { /* skip */ }
                            }
                            exitSelectMode();
                        }}
                        className="shrink-0 h-7 px-2 bg-red-500/15 text-red-400 rounded text-xs font-semibold hover:bg-red-500/25"
                    >
                        Delete
                    </button>
                    <button
                        onClick={exitSelectMode}
                        className="shrink-0 h-7 px-2 bg-white/5 text-text-muted rounded text-xs font-semibold hover:bg-white/10"
                    >
                        Cancel
                    </button>
                </div>
                        ...
                        {!showTools ? (
                            <>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        setEditingConnection(null);
                                        setTemplateConnection(contextMenu.connection);
                                        setShowConnectionDialog(true);
                                        setContextMenu(null);
                                    }}
                                >
                                ...
                                <button
                                    type="button"
                                    className="w-full flex items-center gap-3 px-3 py-2 text-sm text-text-primary hover:bg-white/5 rounded-lg transition-colors"
                                    onClick={() => handleWakeOnLan(contextMenu.connection)}
                                >
                                    <Power className="w-3.5 h-3.5" /> Wake-on-LAN
                                </button>
