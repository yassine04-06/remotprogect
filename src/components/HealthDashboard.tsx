...
export const HealthDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const runPings = async () => {
            ...
            // Collect ping results independently of current state to avoid stale closure
            ...
            if (mounted) {
                // Use functional updater to always merge against latest state (no stale closure)
                setHealthData(prev => {
                    const next = { ...prev };
                    for (const { id, online, latency } of updates) {
                        if (!next[id]) {
                            next[id] = { online, latency, history: [latency] };
                        } else {
                            const history = [...next[id].history, latency].slice(-20); // Keep last 20
                            next[id] = { online, latency, history };
                        }
                    }
                    return next;
                });
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-3">
                        <div
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/5 bg-surface text-xs font-semibold ${isPinging ? 'text-accent' : 'text-text-muted'}`}
                        >
                            <RefreshCw
                    ...
                    {connections.map(conn => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div
                                    className={`absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl opacity-20 transition-colors duration-1000 ${hasData ? (isOnline ? 'bg-green-500' : 'bg-red-500') : 'bg-gray-500'}`}
                                />
