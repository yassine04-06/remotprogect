...
export const CommandLibraryModal: React.FC = () => {
    ...
    useEffect(() => {
        if (!showCommandLibraryDialog) return;
        const handleKey = (e: KeyboardEvent) => {
            if (e.key === 'Escape') setShowCommandLibraryDialog(false);
        };
        window.addEventListener('keydown', handleKey);
        return () => window.removeEventListener('keydown', handleKey);
    }, [showCommandLibraryDialog, setShowCommandLibraryDialog]);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            const appErr = parseBackendError(error);
            addToast({
                type: 'error',
                title: 'Failed to save command',
                description: getUserFriendlyErrorMessage(appErr),
            });
        }
    };
        ...
        >
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-muted">
                                Manage your saved shell snippets and macros
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-hidden flex flex-col lg:flex-row relative">
                    {/* Left Panel: List */}
                    <div
                        className={`flex-1 flex flex-col border-r border-white/5 transition-all ${isCreating || editingCmd ? 'hidden lg:flex' : 'flex'}`}
                    >
