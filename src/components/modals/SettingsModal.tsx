...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
    ...
    // MED-A10: full vault backup (zip of db + config + certs + recordings)
    const handleBackup = async () => {
        try {
            const path = await saveDialog({
                defaultPath: `nexorc_backup_${new Date().toISOString().split('T')[0]}.zip`,
                filters: [{ name: 'NexoRC Backup', extensions: ['zip'] }],
            });
            if (!path) return;
            const n = await api.vaultBackup(path);
            addToast({ type: 'success', title: 'Backup created', description: `${n} files archived.` });
        } catch (err) {
            addToast({ type: 'error', title: 'Backup failed', description: getUserFriendlyErrorMessage(parseBackendError(err)) });
        }
    };

    const handleRestore = async () => {
        try {
            const path = await openDialog({ filters: [{ name: 'NexoRC Backup', extensions: ['zip'] }], multiple: false });
            if (!path || typeof path !== 'string') return;
            const ok = await confirm(
                'Restore will OVERWRITE the current vault (database, config, recordings) and the app will close. Continue?',
                { title: 'Confirm Restore', kind: 'warning' },
            );
            if (!ok) return;
            const n = await api.vaultRestore(path);
            addToast({ type: 'success', title: 'Vault restored', description: `${n} files restored. Restart the app.` });
        } catch (err) {
            addToast({ type: 'error', title: 'Restore failed', description: getUserFriendlyErrorMessage(parseBackendError(err)) });
        }
    };
                                            ...
                                            {[
                                                {
                                                    id: 'auto',
                                                    name: 'Auto (System)',
                                                    desc: 'Follows OS light/dark',
                                                    preview: 'bg-gradient-to-br from-[#1e1e2e] to-[#ffffff]',
                                                },
                                                {
                                                    id: 'default',
                                    ...
                                    >
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <button
                                                ...
                                                className="flex flex-col items-center justify-center p-6 bg-violet-500/5 border border-border rounded-2xl hover:bg-violet-500/10 hover:border-violet-500/30 transition-all group"
                                            >
                                                <div className="w-12 h-12 rounded-xl bg-violet-500/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-all">
                                                    <Archive className="w-6 h-6 text-violet-400" />
                                                </div>
                                                <span className="text-sm font-bold">Full Backup (.zip)</span>
                                                <p className="text-[10px] text-text-muted mt-1">
                                                    DB + config + certs + recordings
                                                </p>
                                            </button>
                                            ...
                                            >
                                                ...
                                                <span className="text-sm font-bold">Restore Backup</span>
                                                <p className="text-[10px] text-text-muted mt-1">
                                                    Overwrites vault · restart required
                                                </p>
                                            </button>
                                        </div>
