...
import { X, Plus, KeyRound, Search, Edit2, Trash2, Sparkles, ShieldCheck, ShieldAlert, Loader2 } from 'lucide-react';
...
export const CredentialManagerModal: React.FC = () => {
    ...
    const checkBreach = async () => {
        ...
        setBreachCount(null);
        try {
            setBreachCount(await api.checkPasswordBreach(password));
        } catch (err) {
            addToast({ type: 'error', title: 'Breach check failed', description: String(err) });
        } finally {
            setBreachBusy(false);
        }
    };
    ...
    useEffect(() => {
        if (!showCredentialManager) return;
        const handleKey = (e: KeyboardEvent) => {
            if (e.key === 'Escape') setShowCredentialManager(false);
        };
        window.addEventListener('keydown', handleKey);
        return () => window.removeEventListener('keydown', handleKey);
    }, [showCredentialManager, setShowCredentialManager]);
    ...
    const handleOpenEdit = (profile: CredentialProfile) => {
        setIsCreating(false);
        ...
        // SECURITY (NXS-005): do NOT decrypt existing credentials into the UI.
        // The user can re-type the password/key to change it; leaving the
        // field blank preserves the existing one (handled in handleSave).
        setPassword('');
        setPrivateKey('');
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // inside create_credential_profile / update_credential_profile.
            // Preserve existing encrypted blobs when editing and the field is blank.
            const req: {
                name: string;
                type: CredentialType;
            ...
            if (password.trim() !== '') {
                ...
                req.password_encrypted = editingProfile.password_encrypted ?? null;
            }

            if (privateKey.trim() !== '') {
                req.private_key_plaintext = privateKey;
            } else if (editingProfile) {
                req.private_key_encrypted = editingProfile.private_key_encrypted ?? null;
            }
            ...
            addToast({ type: 'error', title: 'Save failed', description: String(error) });
        }
    };

    const handleDelete = async (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        const usedCount = connections.filter(
            c => !c.override_credentials && c.credential_profile_id === id
        ).length;
        const msg =
            usedCount > 0
    ...
    const handleDuplicate = async (e: React.MouseEvent, p: CredentialProfile) => {
        ...
        try {
            ...
            addToast({ type: 'error', title: 'Duplicate failed', description: String(err) });
        }
    };
        ...
        >
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-muted">
                                Manage global credential profiles for your connections
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-hidden flex flex-col lg:flex-row relative">
                    {/* Left Panel: List */}
                    <div
                        className={`flex-1 flex flex-col border-r border-white/5 transition-all ${isCreating || editingProfile ? 'hidden lg:flex' : 'flex'}`}
                    >
                                ...
                                filteredProfiles.map(p => (
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-2 text-xs text-text-muted truncate">
                                            <span className="bg-white/5 px-1.5 py-0.5 rounded font-mono uppercase text-[10px]">
                                                {p.type}
                                            </span>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-2">
                                                <input
                                                    type="text"
                                                    value={password}
                                                    onChange={e => { setPassword(e.target.value); setBreachCount(null); }}
                                                    placeholder={
                                                        editingProfile?.password_encrypted
                                                            ? '••••••••  (leave blank to keep current)'
                                                            : ''
                                                    }
                                                    className="flex-1 min-w-0 bg-background rounded border border-white/10 px-3 py-2 text-text outline-none focus:border-accent font-mono"
                                                    style={password ? undefined : { fontFamily: 'inherit' }}
                                                />
                                                <button
                                                    type="button"
                                                    onClick={checkBreach}
                                                    disabled={!password || breachBusy}
                                                    title="Check against Have I Been Pwned (k-anonymity — password never sent)"
                                                    className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-white/5 border border-white/10 text-text-muted rounded text-xs font-semibold hover:bg-white/10 disabled:opacity-40"
                                                >
                                                    {breachBusy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ShieldCheck className="w-3.5 h-3.5" />} Check
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        ...
                                                        addToast({ type: 'success', title: 'Password generated', description: '20-char secure password copied to clipboard.' });
                                                    }}
                                                    title="Generate secure password (copies to clipboard)"
                                                    className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-accent/10 border border-accent/30 text-accent rounded text-xs font-semibold hover:bg-accent/20"
                                                >
                                            ...
                                            {breachCount !== null && (
                                                breachCount > 0 ? (
                                                    <p className="mt-1.5 flex items-center gap-1.5 text-xs text-red-400">
                                                        ...
                                                        Found in {breachCount.toLocaleString()} known breaches — choose another.
                                                    </p>
                                                ) : (
                                        ...
                                        {['ssh', 'ftp', 'generic'].includes(type) && (
                                            <div>
                                                ...
                                                <textarea
                                                    ...
                                                    rows={4}
                                                    placeholder={
                                                        editingProfile?.private_key_encrypted
