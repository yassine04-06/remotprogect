...
import type { PredefinedToolId } from '../services/api';
...
export function ExternalToolsMenu({
    ...
    const tools: ToolEntry[] = [
        { name: 'Ping', id: 'ping', icon: <Activity className="w-3.5 h-3.5" /> },
        { name: 'Traceroute', id: 'traceroute', icon: <Zap className="w-3.5 h-3.5" /> },
        { name: 'DNS Lookup', id: 'dns_lookup', icon: <Terminal className="w-3.5 h-3.5" /> },
    ];

    const runTool = async (toolName: string, toolId: PredefinedToolId) => {
        setIsRunning(toolName);
        try {
            const result = await api.runPredefinedTool(toolId, connection.host);
            if (result.success) {
                addToast({
                    type: 'info',
                ...
                addToast({
                    ...
                    description: result.stderr || 'Command failed',
                });
            }
        } catch (err) {
        ...
        <div className="py-1 min-w-[160px]">
            ...
            {tools.map(tool => (
                <button
                    ...
                    onClick={() => runTool(tool.name, tool.id)}
