...
export function LocalTerminalView({ tab, isActive }: Props) {
    ...
    // M2: asciicast recording for local shells (same recorder as SSH).
    ...
    useEffect(() => {
        return () => { if (isRecording) api.sshRecordingStop(tab.id).catch(() => {}); };
    }, [isRecording, tab.id]);
        ...
        >
            ...
            <div className="absolute top-4 right-6 z-20 flex gap-2">
                <button
                    ...
                    className={`p-2 glass-card rounded-lg border shadow-lg flex items-center gap-2 transition-all ${isRecording ? 'border-red-500/50 text-red-400 hover:bg-red-500/10' : 'border-white/5 text-text-muted hover:bg-white/10 hover:border-accent hover:text-accent'}`}
                >
            ...
            {menuPosition && (
                ...
                >
                    ...
                    >
                        <span className={termRef.current?.hasSelection() ? '' : 'opacity-50'}>
                            Copy
                        </span>
                    </button>
                    <button
