...
export const ProxmoxView: React.FC<Props> = ({ connection }) => {
    ...
    // Auth & Init — prefers API token (90-15) over password auth
    // CRIT-A4: credentials resolved server-side; frontend never sees plaintext.
    ...
    const handleOpenConsole = async (res: ProxmoxResource) => {
        ...
        try {
            ...
            addToast({
                ...
                description: `Connected to ${res.name}`,
            });
        } catch (err: unknown) {
    ...
    const handleAction = async (vmid: string, node: string, type: string, action: string) => {
        ...
        try {
            ...
            addToast({
                type: 'success',
                title: 'Action Sent',
                description: `${action.toUpperCase()} initiated on VM ${vmid}`,
            });
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-text-muted flex gap-4">
                            <span>
                                <strong className="text-text-primary">{resources.length}</strong>{' '}
                                Total Node(s)
                            </span>
                            <span className="text-green-400">
                                <strong className="text-green-400">{runningCount}</strong> Running
                            </span>
                            <span className="text-red-400">
                                <strong className="text-red-400">{stoppedCount}</strong> Stopped
                            </span>
                        </p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-accent/5 text-[10px] uppercase tracking-wider text-text-muted font-bold">
                                ...
                                <th className="p-4 border-b border-border w-1/4">
                                    CPU & RAM Usage
                                </th>
                                <th className="p-4 border-b border-border text-right w-[150px]">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <AnimatePresence>
                                {resources.map(res => {
                                    ...
                                    const memPercent =
                                        res.maxmem && res.mem ? (res.mem / res.maxmem) * 100 : 0;
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="p-4">
                                                <div
                                                    className={`flex items-center gap-2 text-xs font-bold ${isRunning ? 'text-green-400' : 'text-red-400 text-opacity-70'}`}
                                                >
                                                    <div
                                                        className={`w-2 h-2 rounded-full ${isRunning ? 'bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.5)]' : 'bg-red-500/50'}`}
                                                    />
                                                ...
                                                {isRunning ? (
                                                    <div className="flex flex-col gap-2">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[10px] w-8 text-text-muted uppercase font-bold">
                                                                CPU
                                                            </span>
                                                    ...
                                                    <span className="text-[10px] text-text-muted/40 font-bold uppercase">
                                                        — Offline —
                                                    </span>
                                                )}
                                            </td>
                                                ...
                                                <div className="flex items-center justify-end gap-1">
                                                    {isRunning ? (
                                                        <>
                                                            <button
                                                                onClick={() =>
                                                                    handleOpenConsole(res)
                                                                }
                                                                className="p-1.5 rounded-lg text-text-muted hover:text-pink-400 hover:bg-pink-400/10 transition-colors"
                                                                title="Open Console"
                                                            >
                                                            ...
                                                            <button
                                                                ...
                                                                className="p-1.5 rounded-lg text-text-muted hover:text-orange-400 hover:bg-white/5 transition-colors"
                                                                title="Reboot"
                                                            >
                                                                {actionLoading ===
                                                                res.id.split('/')[1] ? (
