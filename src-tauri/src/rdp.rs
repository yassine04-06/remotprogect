...
pub fn ensure_helper_compiled(data_dir: &str) -> Result<String, String> {
    ...
    let pre_compiled_candidates = {
        ...
        if let Some(dev_pre) = exe_dir
            .parent()
            .and_then(|p| p.parent())
            .map(|p| p.join("helpers").join("RdpEmbed.exe"))
        {
            v.push(dev_pre);
        }
        v
    ...
    // No pre-compiled binary found — check cache before trying to compile.
...
pub fn launch_rdp_mstsc(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let rdp_file = temp_dir.join(format!("nexorc_{}.rdp", uuid::Uuid::new_v4()));
        ...
        let rdp_file_str = rdp_file.to_str().ok_or("Percorso file .rdp non UTF-8")?;
