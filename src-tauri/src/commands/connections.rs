...
macro_rules! rate_check {
    ($state:expr, $cmd:literal) => {
        $state.command_limiter.check_key(&$cmd).map_err(|_| {
            crate::error::AppError::RateLimit(concat!(
                "Too many requests for '", $cmd, "' — slow down"
            ).to_string())
        })?
    };
}
...
pub fn create_connection(
    ...
    let _ = database::audit_log_insert(&conn, "create", "connection", &result.id, &result.name, "success", "");
    Ok(result)
}
...
#[tauri::command]
pub fn delete_connection(state: tauri::State<AppState>, id: String) -> Result<(), crate::error::AppError> {
    let conn = state.db.get().map_err(|e| crate::error::AppError::Internal(format!("DB pool: {}", e)))?;
...
#[tauri::command]
pub fn get_connections(state: tauri::State<AppState>) -> Result<Vec<database::ServerConnection>, crate::error::AppError> {
...
#[tauri::command]
pub fn get_connections_summary(state: tauri::State<AppState>) -> Result<Vec<ConnectionSummary>, crate::error::AppError> {
...
#[tauri::command]
pub async fn check_for_update(app: tauri::AppHandle) -> Result<serde_json::Value, crate::error::AppError> {
    ...
    match app.updater() {
        Ok(updater) => match updater.check().await {
            ...
            Err(e) => Err(crate::error::AppError::Network(format!("Update check failed: {}", e))),
        },
        Err(e) => Err(crate::error::AppError::Internal(format!("Updater not configured: {}", e))),
    }
}
