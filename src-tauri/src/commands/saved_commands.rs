...
#[tauri::command]
pub fn get_saved_commands(state: tauri::State<AppState>) -> Result<Vec<database::SavedCommand>, crate::error::AppError> {
    let conn = state.db.get().map_err(|e| crate::error::AppError::Internal(format!("DB pool: {}", e)))?;
...
#[tauri::command]
pub fn delete_saved_command(state: tauri::State<AppState>, id: String) -> Result<(), crate::error::AppError> {
    let conn = state.db.get().map_err(|e| crate::error::AppError::Internal(format!("DB pool: {}", e)))?;
